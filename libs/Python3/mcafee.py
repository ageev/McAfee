...
class ExHTTPSConnection(http.client.HTTPSConnection):
...
"""
class ExHTTPSHandler(urllib.request.HTTPSHandler):
...
#        passmgr.add_password(None, self.baseurl, username.encode(_UTF8), password.encode(_UTF8))
        ...
        if sys.version_info < (2, 7):
            ...
            self.opener = urllib.request.build_opener(ExHTTPSHandler(), authhandler, urllib.request.HTTPCookieProcessor())
    ...
    def invoke(self, command, args={}):
        ...
        for key in argscopy:
            arg = argscopy[key]
            if type(arg) != type('') and type(arg) != type(''): continue #skip non-strings
            if arg[:8] == 'file:///':
                filename = arg[8:]
    ...
    def build_url_request(self, command, args):
        ...
        query_string = "&".join(["%s=%s" % (quote(str(k)), quote(str(v))) for k, v in list(args.items())])
        if not query_string:
            return url
